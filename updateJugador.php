...
// Recuperar datos
$jugadores = getJugador($id, $pdo);

$sql = "SELECT * from jugador WHERE id = :id LIMIT 1";
...
$sqlLiga = "SELECT liga.* from liga, equipo, jugador WHERE jugador.id =:id AND jugador.equipo=equipo.nombre AND equipo.liga = liga.nombre LIMIT 1";
$resultLiga = $pdo->prepare($sqlLiga);
$resultLiga->execute([
    'id' => $id
]);

$liga = $resultLiga->fetch(PDO::FETCH_ASSOC);
...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        ...
        // Guardo en la BD
        $sql = "UPDATE jugador SET imagen = :imagen, nombre = :nombre, apellido = :apellido, edad = :edad, altura = :altura, peso = :peso, posicion = :posicion, partidos = :partidos, ensayos = :ensayos, amarillas = :amarillas, rojas = :rojas, equipo = :equipo WHERE id = :id LIMIT 1";
...
<nav class="navbar navbar-inverse navbar-fixed-top">
    ...
    <div class="container">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="equipo.php?id=<?= $equipo['id'] ?>">Equipo</a><span class="navbar-brand"> | </span>
            <a class="navbar-brand" href="jugador.php?id=<?= $row['id'] ?>">Jugador</a><span class="navbar-brand"> | </span>
            <a class="navbar-brand btn btn-primary btn-lg active" href="updateJugador.php?id=<?= $row['id'] ?>">Modificar Jugador</a>
        </div>
